...
/*This script is for editting a quote.*/
...
if (isset($_GET['id']) && is_numeric($_GET['id']) && ($_GET['id'] > 0)) {
    ...
    //Make the form:
        print '<form action="edit_quote.php" method="post">
              ...
              <p><label>Is this a favorite? <input type="checkbox" name="favorite" value="yest"';
  ...
  if (!$problem){
    ...
    if ($result = mysqli_query($dbc, $query)){
      ...
      print '<p class="error">Could not update the quotation because:<br>' . mysqli_error($dbc) . '.</p><p>The query being run was: ' . $qery . '</p>';
    }
  }// No problem!
} else { // No ID set.

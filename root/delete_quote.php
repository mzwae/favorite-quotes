<?php
/*This script is to delete a quote*/
...
if (!is_administrator()) {
    print '<h2>Access Denied!</h2>
         <p class="error">You do not hae permission to access this page.</p>';
...
if (isset($_GET['id']) && is_numeric($_GET['id']) && ($_GET['id'] > 0)) {
    ...
    if (mysqli_affected_rows($dbc) == 1) {
        ...
        print '<p class="error">Could not delete the quote entry because:<br>' . mysqli_error($dbc) . '.</p><p>The query being run was: ' . $query . '</p>';
    }
} else {
    // No ID received.
